...
export { GAME_SESSION_VERSION, gameSessionSummary } from './gameSessionSummary'
...
export function saveGameSession(
  ...
  try {
    storage.setItem(gameSessionKey(snapshot.mode), JSON.stringify(snapshot))
    return true
  } catch {
    return false
  }
...
export function clearGameSession(
  ...
  if (!storage) return false
  try {
    storage.removeItem(gameSessionKey(mode))
    return true
  } catch {
    return false
  }
